...
Turret_Args["coaxial"] = function(turret, target) turret.coaxial=number_to_boolean(target) end
...
-- Int_Arguments
Turret_Args["crew"] = function(turret, target) turret.crew=Crew():add(math.max(math.floor(target), 1), CrewMan(CrewProfessionType.Gunner)) end
...
Weapon_Args["weaponsize"] = function(weapon, target) if weapon.isBeam then weapon.bshapeSize,weapon.bwidth,weapon.blength,wepaon.bauraWidth=weapon.bshapeSize*target,weapon.bwidth*target,weapon.blength*target,weapon.bauraWidth*target else weapon.psize=weapon.psize*target end end

-- Absolute_Arguments
Weapon_Args["accuracy"] = function(weapon, target) weapon.accuracy=target end
Weapon_Args["efficiency"] = function(weapon, target) weapon.stoneRefinedEfficiency,weapon.metalRefinedEfficiency,weapon.stoneRawEfficiency,weapon.metalRawEfficiency = target,target,target,target end
						...
						if tonumber(amount) then
							...
							if randomized then
								for i=1,amount do
									...
									if barrels then
										TurretGenerator.attachWeapons(rand, item, weapon, found_args['turret']['barrels'])
									else
										TurretGenerator.attachWeapons(rand, item, weapon, numWeapons)
									end
...
function getDescription()
	return "DESCRIPTION: Used for testing for various experimental effects"
end
